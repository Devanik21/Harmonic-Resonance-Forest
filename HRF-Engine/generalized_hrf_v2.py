...
class HarmonicResonanceClassifier_BEAST_14D(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        # --- STEP B: FIT THE REST OF THE BEASTS (FIXED FALLBACK OVERSIGHT) ---
        if self.verbose:
            print(" > Phase 2: Deploying Logic & Physics Units...")

        unit_names = [
            "unit_01", "unit_02", "unit_03", "unit_04", "unit_05",
            "unit_06", "unit_07", "unit_08", "unit_09", "unit_10", "unit_11"
        ]
        ...
        for name in unit_names:
            ...
            try:
                ...
                if self.verbose: 
                    print(f"   [Warning] {name} adaptation issue: {e}. Falling back to stable SVC.")
                fallback_unit = SVC(kernel='rbf', C=1.0, probability=True, random_state=42)
                fallback_unit.fit(X_evo_t, y_evo_t)
                setattr(self, name, fallback_unit)
                
            other_units.append(getattr(self, name))
