...
#     def __init__(self, k=15, seed=42):
#         self.seed = seed
...
#     def evolve(self, X_val, y_val, patience=10, stagnation=0, generations=50):
#         n_universes = 50
...
#                 traits = random.sample(list(mutant.keys()), n_mutations)
                
#                 for trait in traits:
#                     if trait == 'freq':
#                         mutant['freq'] *= np.random.uniform(0.8, 1.25)
#                     elif trait == 'gamma':
#                         mutant['gamma'] = 10 ** np.random.uniform(-3, 2)
#                     elif trait == 'power':
#                         mutant['power'] = random.choice([0.5, 1, 2, 3, 4, 6])
#                     elif trait == 'p':
#                         mutant['p'] = np.clip(mutant['p'] + np.random.uniform(-0.5, 0.5), 0.5, 8.0)
#                     elif trait == 'phase':
#                         mutant['phase'] = np.random.uniform(0, np.pi)
...
#         """Primary routing function. Directs to GPU if available, else CPU."""
#         if GPU_AVAILABLE:
#             try:
#                 return self._predict_proba_gpu(X)
#             except Exception as e:
#                 warnings.warn(f"GPU prediction failed: {e}. Falling back to CPU mode.")
...
#             # VRAM OPTIMIZATION: Matrix Formulation for Euclidean Distance
#             if p_norm == 2.0:
#                 X_sq = cp.sum(batch_te ** 2, axis=1, keepdims=True)
#                 Y_sq = cp.sum(X_tr_g ** 2, axis=1)
#                 cross = cp.dot(batch_te, X_tr_g.T)
                
#                 # Calculate distance squared, ensuring no negative values due to float precision
#                 dists_sq = cp.maximum(X_sq + Y_sq - 2.0 * cross, 0.0)
#                 dists = cp.sqrt(dists_sq)
            
#             # FALLBACK: Chunked loop for non-Euclidean distances 
#             else:
#                 train_chunk_size = 512
#                 dists = cp.empty((len(batch_te), n_train), dtype=cp.float32)

#                 for j in range(0, n_train, train_chunk_size):
#                     end_tr = min(j + train_chunk_size, n_train)
#                     chunk_tr = X_tr_g[j:end_tr]

#                     diff = cp.abs(batch_te[:, None, :] - chunk_tr[None, :, :])
#                     chunk_dists = cp.sum(cp.power(diff, p_norm), axis=2)
#                     dists[:, j:end_tr] = cp.power(chunk_dists, 1.0 / p_norm)
...
#             del batch_te, dists, top_k_idx, top_dists, w, cosine_term
#             if p_norm != 2.0:
#                 del diff
...
#             # OPTIMIZATION: Matrix Formulation for Euclidean Distance
#             if p_norm == 2.0:
#                 X_sq = np.sum(batch_te ** 2, axis=1, keepdims=True)
#                 Y_sq = np.sum(X_train ** 2, axis=1)
#                 cross = np.dot(batch_te, X_train.T)
                
#                 dists_sq = np.maximum(X_sq + Y_sq - 2.0 * cross, 0.0)
#                 dists = np.sqrt(dists_sq)
            
#             # FALLBACK: Chunked loop for non-Euclidean distances 
#             else:
#                 train_chunk_size = 512
#                 dists = np.empty((len(batch_te), n_train), dtype=np.float32)

#                 for j in range(0, n_train, train_chunk_size):
#                     end_tr = min(j + train_chunk_size, n_train)
#                     chunk_tr = X_train[j:end_tr]

#                     diff = np.abs(batch_te[:, None, :] - chunk_tr[None, :, :])
#                     chunk_dists = np.sum(np.power(diff, p_norm), axis=2)
#                     dists[:, j:end_tr] = np.power(chunk_dists, 1.0 / p_norm)
...
#         self.unit_12 = HolographicSoulUnit(k=15, seed=42) # Will evolve independently

#         # 13. TWIN SOUL ALPHA (Mirror 1 - Chaos Seed)
#         self.unit_13 = HolographicSoulUnit(k=15, seed=1337) # Will evolve independently

#         # 14. TWIN SOUL BETA (Mirror 2 - Order Seed)
#         self.unit_14 = HolographicSoulUnit(k=15, seed=9001) # Will evolve independently
...
#             best_acc = soul.evolve(X_evo_v, y_evo_v, generations=25)
...
#                     p = unit.predict_proba(X_holdout)
...
#             ll = log_loss(y_holdout, np.clip(final_p, 1e-15, 1-1e-15))
#             lambda_reg = 0.05
#             reg = np.sum(w ** 2)
#             return ll + lambda_reg * reg

#         # Smart Initialization based on individual performance
#         accuracies = [
#             accuracy_score(
#                 y_holdout,
#                 self.classes_[np.argmax(p, axis=1)]
#                 )
...
class HolographicSoulUnit(BaseEstimator, ClassifierMixin):
    ...
    # 🚨 HERO ROUTE FIX: Project Test Data to match Training Data dimensions 🚨
    def predict_proba(self, X):
        """Primary routing function. Directs to GPU if available, else CPU."""
        
        # If we reduced dimensions in fit(), we MUST reduce them here too
        if self.projector_ is not None:
            X_processed = self.projector_.transform(X)
        else:
            X_processed = X

        if GPU_AVAILABLE:
            try:
                return self._predict_proba_gpu(X_processed)
            except Exception as e:
                warnings.warn(f"GPU prediction failed: {e}. Falling back to CPU mode.")
                return self._predict_proba_cpu(X_processed)
        else:
            return self._predict_proba_cpu(X_processed)
